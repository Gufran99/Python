...
users = [gufran_account, mustafa_account, umran_account]
...
def eft_process(sender_account: dict, receiver_account_no: str, amount: int):
    ...
    if process_is_valid:
        if amount >= sender_account['ballance']:
            withdraw_money(sender_account, amount)
        else:
            sender_account['ballance'] -= amount
